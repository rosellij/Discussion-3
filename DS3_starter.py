...
class Dice:
    ...
    def __str__(self):
        ...
        return 'Last roll: {}'.format(self.rolls[-1])
    ...
    def roll(self):
        ...
        selection = random.randint(1, self.sides)
        self.rolls += [selection]
        return selection
    ...
    def num_rolls(self):
        ...
        userinput = int(input('How many times do you want to roll?'))
        for anynum in range(userinput):
            result = self.roll()
            print(result)
    ...
    def print_count_for_num(self, number):
        ...
        print('number was rolled {} times'.format(self.rolls.count(number)))
